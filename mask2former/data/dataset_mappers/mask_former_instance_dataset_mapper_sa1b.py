...
from mask2former.utils import detection_utils as utils
from detectron2.structures import BoxMode
from detectron2.data import transforms as T
from detectron2.projects.point_rend import ColorAugSSDTransform
from detectron2.structures import BitMasks, Instances, polygons_to_bitmask

__all__ = ["MaskFormerInstanceDatasetMapperSA1B"]


class MaskFormerInstanceDatasetMapperSA1B:
    """
    ...
    def __call__(self, dataset_dict):
        ...
        assert "ann_file" in dataset_dict
        ann_file = dataset_dict["ann_file"]
        annotations = json.load(open(ann_file))

        annos = [
            obj for obj in annotations["annotations"] if obj.get("iscrowd", 0) == 0
        ]
        ...
        masks = [
            (
                ...
                else torch.from_numpy(np.ascontiguousarray(x.copy()))
            )
            for x in masks
        ]

        classes = [0 for obj in annos]
