...
def load_sa1b_segments(image_dir, annotations_dir, meta):
    ...
    _logger.info(f"Found {len(images)} images in {image_dir}")
    _logger.info(f"Found {len(annotations)} annotations in {annotations_dir}")
    if len(annotations) != len(images):
        for ann in annotations:
            image_id_ann = ann.split(".")[0]
            image_file = os.path.join(image_dir, f"{image_id_ann}.jpg")
            assert PathManager.isfile(
                image_file
            ), f"Image file not found: {image_file}"
            ann_file = os.path.join(annotations_dir, ann)
            _logger.info(f"Found {image_file} and {ann_file}")
            ret.append(
                {
...
_PREDEFINED_SPLITS_SA1B_SEGMENTS = {
    "vlm_ade_whole_sa1b_train": (
        "ADEChallengeData2016/images/training",
        "sam_labels_ade/points_per_side_32_pred_iou_thresh_0.88_stability_score_thresh_0.95/instance_anns",
    ),
    "vlm_ade_whole_sa1b_train_unsamplus_08": (
        "ADEChallengeData2016/images/training",
        "ADEChallengeData2016/unsam_plus_conf0.8/points_per_side_32_pred_iou_thresh_0.88_stability_score_thresh_0.95/instance_anns",
    ),
    "vlm_ade_whole_sa1b_train_unsamplus_05": (
        "ADEChallengeData2016/images/training",
        "ADEChallengeData2016/unsam_plus_conf0.5/points_per_side_32_pred_iou_thresh_0.88_stability_score_thresh_0.95/instance_anns",
    ),
    "vlm_ade_whole_sa1b_train_unsam_08": (
        "ADEChallengeData2016/images/training",
        "ADEChallengeData2016/unsam_conf0.8/points_per_side_32_pred_iou_thresh_0.88_stability_score_thresh_0.95/instance_anns",
    ),
    "vlm_ade_whole_sa1b_train_unsam_05": (
        "ADEChallengeData2016/images/training",
        "ADEChallengeData2016/unsam_conf0.5/points_per_side_32_pred_iou_thresh_0.88_stability_score_thresh_0.95/instance_anns",
    ),
}
