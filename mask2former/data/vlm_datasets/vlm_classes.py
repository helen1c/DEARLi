...
SA1B_CATEGORIES = [
    {"color": [150, 150, 150], "id": 0, "isthing": 1, "name": "segment"},
]
...
def get_ade20k_categories_with_prompt_eng():
    ...
    ade20k_id_names = (
        open("./mask2former/data/vlm_datasets/eng_prompts/ade20k_150_prompts.txt")
        .read()
        .splitlines()
    )
    ade_idx = 0
...
def get_coco_categories_with_prompt_eng():
    ...
    coco_id_names = (
        open("./mask2former/data/vlm_datasets/eng_prompts/coco_panoptic_prompts.txt")
        .read()
        .splitlines()
    )
    coco_idx = 0
...
def get_coco_objects_categories_with_prompt_eng():
    COCO_OBJ_CATEGORIES_ = copy.deepcopy(COCO_OBJ_CATEGORIES)
    coco_id_names = (
        open("./mask2former/data/vlm_datasets/eng_prompts/coco_objects_prompts.txt")
        .read()
        .splitlines()
    )
    coco_idx = 0
    for line in coco_id_names:
        ...
        if name == "invalid_class_id":
            continue
        # # print(COCO_CATEGORIES_[coco_idx]["name"], '->', name)
        assert COCO_OBJ_CATEGORIES_[coco_idx]["id"] == idx
        COCO_OBJ_CATEGORIES_[coco_idx]["name"] = name
        coco_idx += 1
    return COCO_OBJ_CATEGORIES_


def get_sa1b_categories_with_prompt_eng():
    SA1B_CATEGORIES_ = copy.deepcopy(SA1B_CATEGORIES)
    sa1b_id_names = (
        open("./mask2former/data/vlm_datasets/eng_prompts/sa1b_prompts.txt")
        .read()
        .splitlines()
    )
    sa1b_idx = 0
    for line in sa1b_id_names:
        idx, name = line.split(":")
        idx = int(idx)
        if name == "invalid_class_id":
            continue
        assert SA1B_CATEGORIES_[sa1b_idx]["id"] == idx
        SA1B_CATEGORIES_[sa1b_idx]["name"] = name
        sa1b_idx += 1
    return SA1B_CATEGORIES_
