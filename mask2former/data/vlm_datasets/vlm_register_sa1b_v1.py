...
def load_sa1b_segments(image_dir, annotations_dir, meta):
    ...
    image_tars = sorted(os.listdir(image_dir))
    ann_tars = sorted(os.listdir(annotations_dir))
    print(image_tars, ann_tars)
    for image_tar, ann_tar in zip(image_tars, ann_tars):
        annotations_dir_ = os.path.join(annotations_dir, ann_tar, "annotations")
        image_dir_ = os.path.join(image_dir, image_tar, "images")
        annotations = sorted(os.listdir(annotations_dir_))
        images = sorted(os.listdir(image_dir_))
        ...
        for ann, img in zip(annotations, images):
            ...
            image_file = os.path.join(image_dir_, img)
            ann_file = os.path.join(annotations_dir_, ann)
            ret.append(
                {
