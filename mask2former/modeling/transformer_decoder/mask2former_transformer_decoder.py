...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None):
        ...
        outputs_class, outputs_mask, attn_mask, decoder_output = (
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_decoder):
        ...
        if self.mask_classification:
            return [
                ...
                for a, b, c in zip(
                    outputs_class[:-1], outputs_seg_masks[:-1], outputs_decoder[:-1]
                )
            ]
        else:
            return [
                {"pred_masks": b, "decoder_output": c}
                for b, c in zip(outputs_seg_masks[:-1], outputs_decoder[:-1])
            ]
