...
class VisionLanguageMaskFormer(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if "convnext" in backbone.model_name.lower():
                pooled_clip_feature = self.mask_pooling(clip_feature, mask_for_pooling)
                pooled_clip_feature = backbone.visual_prediction_forward(
                    pooled_clip_feature
                )
            elif "rn" in backbone.model_name.lower():
                pooled_clip_feature = backbone.visual_prediction_forward(
                    clip_feature, mask_for_pooling
                )
            elif "vit" in backbone.model_name.lower():
                pooled_clip_feature = self.mask_pooling(clip_feature, mask_for_pooling)
                pooled_clip_feature = backbone.visual_prediction_forward(
                    pooled_clip_feature
                )
            ...
            out_vocab_cls_results = get_classification_logits(
                pooled_clip_feature,
                text_classifier,
                backbone.clip_model.logit_scale,
                num_templates,
            )
